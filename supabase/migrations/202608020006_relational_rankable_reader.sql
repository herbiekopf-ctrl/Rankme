-- Service-only reader used by the Next.js API. One RPC reconstructs a versioned
-- rankable dataset without exposing the server secret or requiring thousands of
-- PostgREST round trips from the browser.

create or replace function public.get_rankable_dataset(
  p_season integer,
  ...
  select jsonb_build_object(
    ...
    'metrics', coalesce((
      select jsonb_agg(jsonb_build_object(
        'key', ad.key,
        'label', ad.label,
        'description', coalesce(ad.description, ''),
        'valueType', ad.value_type,
        'unit', ad.unit,
        'metricGroup', ad.metric_group,
    ...
    'entities', coalesce((
      select jsonb_agg(jsonb_build_object(
        ...
        'color', e.color,
        'aliases', coalesce((
          select jsonb_agg(ea.alias order by ea.alias) from public.entity_aliases ea where ea.entity_id = e.id
        ), '[]'::jsonb),
        'externalIds', coalesce((
          select jsonb_object_agg(eei.source_slug, eei.external_id) from public.entity_external_ids eei where eei.entity_id = e.id
        ), '{}'::jsonb),
        'attributes', coalesce((
