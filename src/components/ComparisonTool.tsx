...
export function ComparisonTool({ dataset, selectedIds, rankedIds = [], customMetrics = [], canRank = true, onSelectedIdsChange, onOpenEntity = () => undefined, onAddEntity = () => undefined, onClose }: {
  dataset: DatasetEnvelope; selectedIds: string[]; rankedIds?: string[]; customMetrics?: UserCustomMetric[];
  canRank?: boolean; onSelectedIdsChange: (ids: string[]) => void; onOpenEntity?: (id: string) => void; onAddEntity?: (id: string) => void; onClose: () => void;
}) {
  ...
  return <section className="comparison-tool" aria-labelledby="comparison-title">
    ...
    {selected.length ? <div className="metric-table-wrap"><table className="metric-table heatmap-table"><thead><tr><th>Team</th><th>Rank</th>{visibleMetrics.map((metric) => <th key={metric.key}><span>{metric.label}</span><small>{metric.direction === "asc" ? "Lower is better" : "Higher is better"}</small></th>)}</tr></thead><tbody>{selected.map((entity) => <tr key={entity.id}><th><button className="comparison-team-link" onClick={() => onOpenEntity(entity.id)}><TeamMark entity={entity} /><span>{entity.name}</span></button></th><td>{rankedSet.has(entity.id) ? `#${rankedIds.indexOf(entity.id) + 1}` : <button className="compact-rank-action" disabled={!canRank} onClick={() => onAddEntity(entity.id)}>{canRank ? "+ Rank" : "Closed"}</button>}</td>{visibleMetrics.map((metric) => { const value = valueFor(entity, metric); const heat = heatFor(entity, metric); return <td key={metric.key} className={heat == null ? "is-missing" : "heat-cell"} style={heat == null ? undefined : { "--heat": heat } as React.CSSProperties}><strong>{formatMetricValue(value, metric)}</strong></td>; })}</tr>)}</tbody></table></div> : <div className="comparison-empty"><strong>Select teams from your ranking or candidate pool.</strong><span>You can compare two, all 25, or anything in between.</span></div>}
  </section>;
}
